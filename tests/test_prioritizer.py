...
class Testing(unittest.TestCase):
    def test_prioritizer(self):
        prioritizer = Prioritizer(my_weights=my_layer, alpha=.4, max_selected=4)
        ...
        low_qual = prioritizer.select_qualified(sorted_distance_map, .45)
        assert('1a' not in list(low_qual.keys()))
        high_qual = prioritizer.select_qualified(sorted_distance_map, .45, True)
        assert('3' not in list(high_qual.keys()))
        prioritized = prioritizer.get_prioritized_weights(layer_map)
        print(prioritized)
        assert('1a' not in list(prioritized.keys()))
        assert('6' not in list(prioritized.keys()))
